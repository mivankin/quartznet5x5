...
class QuartznetModule(LightningModule):
    def __init__(
        ...
        self.encoder = TextEncDec()
        self.greedy_decoder = GreedyCTCDecoder(self.encoder.id_to_token)
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        step_loss = 0
        blank_token = self.encoder.id_to_token[0]
        for idx in indices:
            ...
            for j in range(len(answ[idx])):
                ans = self.encoder.decode(answ[idx][j].cpu().numpy())
                targets = ''.join(ans).replace(blank_token, '')
    ...
    def configure_optimizers(self):
        optimizer = torch_optimizer.NovoGrad(self.parameters(),
                                             lr=0.01,
                                             betas=(0.8, 0.5),
                                             weight_decay=0.001,
                                             )
