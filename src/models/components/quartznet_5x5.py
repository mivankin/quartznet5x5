...
def conv_bn_act(input_dim, output_dim, kernel_size, stride=1, dilation=1):
    block = nn.Sequential(
...
def sepconv_bn(input_dim, output_dim, kernel_size, stride=1, dilation=1, padding=None):
    if padding is None:
...
class QuartzNet(nn.Module):
    def __init__(self, n_mels=64, num_classes=34):
        super().__init__()
